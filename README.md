...
- **Secure Permissions System**: Leveraging Deno's security model, plugins must explicitly request permissions (e.g., network access, file system). You have full control to grant or deny these requests.
- **Instant Access**: DG runs in the background, ensuring it appears instantly when you need it.
- **Built-in Tools**: Comes with an application launcher, calculator, Firefox history search, and a built-in plugin store to discover community extensions.
...
Use `Ctrl+Q` to quit the application completely, or `Escape` to hide it to the background.

## Plugin Development

Plugins are TypeScript modules that implement the `WorkerPlugin` interface. They run in a separate worker thread and can be easily shared by publishing a package to [JSR](https://jsr.io).

To make your plugin discoverable in the built-in store, ensure your JSR package name starts with the `dg-plugin-` prefix (e.g., `@my-scope/dg-plugin-weather`).
