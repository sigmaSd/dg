import { AdwApplicationWindow, MessageDialog } from "@sigmasd/gtk";
import { PluginPermissions } from "../plugins/interface.ts";

export function normalizePermissions(p: PluginPermissions): any {
  const sorted: any = {};
...
export function formatPermissions(p: PluginPermissions): string {
  ...
  if ((p as any).ffi === true) text += "• FFI (Foreign Function Interface)\n\n";
  if ((p as any).hrtime === true) text += "• High Resolution Time\n\n";
  
  return text.trim() || "No special permissions requested.";
}

export async function promptPermissions(window: AdwApplicationWindow, name: string, perms: PluginPermissions): Promise<boolean> {
  return new Promise((resolve) => {
    const formatted = formatPermissions(perms);
    const dialog = new MessageDialog(window, "Permission Request", `The plugin '${name}' requests the following permissions:\n\n${formatted}`);
