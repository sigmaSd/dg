...
- **`csd-demo.ts`**: Demonstrates Client-Side Decorations:
  - Using `AdwApplicationWindow` and `ToolbarView`
  - Toggling system decorations
...
# CSD demo
deno run --allow-ffi examples/csd-demo.ts
```
...
- `AdwWindow` - Adwaita window (supports CSD)
- `AdwApplicationWindow` - Adwaita application window (supports CSD)
...
You can create modern applications with Client-Side Decorations (CSD) using
`AdwApplicationWindow` and `HeaderBar`. This allows you to have a single
titlebar that integrates with your application's content.
...
const app = new Application("com.example.CSD", 0);

app.onActivate(() => {
  const win = new AdwApplicationWindow(app);
  win.setTitle("CSD Window");
  win.setDefaultSize(400, 300);
...
You can also remove system decorations from standard windows:

```typescript
const win = new ApplicationWindow(app);
win.setDecorated(false); // Removes system titlebar and borders
```
